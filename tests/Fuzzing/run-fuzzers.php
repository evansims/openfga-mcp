...
if (!is_dir($corpusDir)) {
    mkdir($corpusDir, 0755, true);
}
if (!is_dir($crashDir)) {
    mkdir($crashDir, 0755, true);
}
...
function mutateInput(string $input): string {
    ...
    switch ($strategy) {
        ...
        case 5: // Shuffle parts
            $parts = str_split($input, max(1, intval(strlen($input) / 4)));
            shuffle($parts);
            $input = implode('', $parts);
            break;
    }
...
foreach ($targetFiles as $targetFile) {
    ...
    if (!is_dir($targetCorpusDir)) {
        mkdir($targetCorpusDir, 0755, true);
    }
    ...
    $targetTimeout = intval($timeout / count($targetFiles));
    $startTime = time();
    ...
    while (time() < $endTime && $iterations < $maxIterations) {
        ...
        if (!empty($corpus) && random_int(0, 1) === 0) {
